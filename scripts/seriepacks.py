...
for i in list.items:
 if i.metadata.name.startswith("r1"):
  ...
  print('Métrica: Paquetes recibidos (bytes)')
...
print('\nSerie:')
while ind<len(dts)-1:
  print(str(dts[ind][0])+' || '+str(round(((dts[ind+1][1])-(dts[ind][1])))))
  dts2.append([(dts[ind][0]),round(((dts[ind+1][1])-(dts[ind][1])))])
...
for i in dts2:
 plo.append(int(i[1]))
print(ace.plot(plo,{'height':10,'offset':10,'format':'{:8.0f}'	
 }))
